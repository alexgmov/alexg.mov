...
let pool;
let sidestreamTelemetryPool;
let sidestreamTelemetryPoolConnectionString;
...
function isDatabaseConfigured(env = process.env) {
  ...
  return !isPlaceholderDatabaseUrl(url);
}

function getSupabaseRestUrl(env = process.env) {
  return env.SUPABASE_URL ||
    env.SIDESTREAM_SUPABASE_URL ||
    '';
}

function getSupabaseRestSecretKey(env = process.env) {
  return env.SUPABASE_SECRET_KEY ||
    env.SUPABASE_SERVICE_ROLE_KEY ||
    '';
}

function isSupabaseRestConfigured(env = process.env) {
  const url = getSupabaseRestUrl(env);
  const key = getSupabaseRestSecretKey(env);
  return Boolean(
    url &&
    key &&
...
function isNeonConnectionString(connectionString) {
  ...
  try {
    ...
    return /\.neon\.tech(?::|\/|$)/i.test(String(connectionString));
  }
}

function getSidestreamTelemetryDatabaseConfig(env = process.env) {
  const directNeonEnvNames = [
    'SIDESTREAM_NEON_DATABASE_URL',
    'NEON_DATABASE_URL',
  ];
  const fallbackEnvNames = [
    'DATABASE_URL',
    'POSTGRES_URL',
  ];

  for (const envName of directNeonEnvNames) {
    const connectionString = env[envName] || '';
    if (!isPlaceholderDatabaseUrl(connectionString)) {
      return { envName, connectionString, collector: 'postgres' };
    }
  }

  for (const envName of fallbackEnvNames) {
    const connectionString = env[envName] || '';
    if (!isPlaceholderDatabaseUrl(connectionString) && isNeonConnectionString(connectionString)) {
      return { envName, connectionString, collector: 'postgres' };
    }
  }
...
function isLegacySupabaseTelemetryEnabled(env = process.env) {
  return env.SIDESTREAM_ALLOW_LEGACY_SUPABASE_TELEMETRY === '1';
}

function getSidestreamTelemetryCollectorConfig(env = process.env) {
  const databaseConfig = getSidestreamTelemetryDatabaseConfig(env);
  if (databaseConfig) {
    return {
      type: 'postgres',
      collector: databaseConfig.collector,
      envName: databaseConfig.envName,
      legacy: false,
    };
  }

  if (isLegacySupabaseTelemetryEnabled(env) && isSupabaseRestConfigured(env)) {
    return {
      type: 'supabase-rest',
      collector: 'supabase-rest',
      envName: 'SIDESTREAM_ALLOW_LEGACY_SUPABASE_TELEMETRY',
      legacy: true,
    };
  }
...
function getSidestreamTelemetryPool() {
  const databaseConfig = getSidestreamTelemetryDatabaseConfig();
  if (!databaseConfig) return null;

  const connectionString = normalizeConnectionString(databaseConfig.connectionString);
  if (!sidestreamTelemetryPool || sidestreamTelemetryPoolConnectionString !== connectionString) {
    sidestreamTelemetryPool = new Pool({
      connectionString,
      max: Number(process.env.POSTGRES_POOL_MAX || 3),
      idleTimeoutMillis: 10000,
      connectionTimeoutMillis: 5000,
    ...
    sidestreamTelemetryPoolConnectionString = connectionString;
  }
  return sidestreamTelemetryPool;
}
...
async function withSidestreamTelemetryClient(fn) {
  const db = getSidestreamTelemetryPool();
  if (!db) return { skipped: true };
  const client = await db.connect();
  try {
    return await fn(client);
...
async function recordPluginTelemetryBatchViaPostgres({
  events,
  req,
  endpointVersion = '2026-06-12.1',
  ...
  return clientRunner(async client => {
    let recorded = 0;
    await client.query('begin');
    try {
      for (const event of normalizedEvents) {
...
async function recordPluginTelemetryBatch({ events, req, endpointVersion = '2026-06-12.1' }) {
  const collectorConfig = getSidestreamTelemetryCollectorConfig();
  if (!collectorConfig) {
    return { skipped: true, recorded: 0 };
  }

  if (collectorConfig.type === 'supabase-rest') {
    return recordPluginTelemetryBatchViaSupabaseRest({ events, req, endpointVersion });
  }

  return recordPluginTelemetryBatchViaPostgres({
    events,
    req,
    endpointVersion,
    clientRunner: withSidestreamTelemetryClient,
    collector: collectorConfig.collector,
  });
}
...
module.exports = {
  ...
  recordPluginTelemetryBatchViaPostgres,
  recordPluginTelemetryBatchViaSupabaseRest,
  recordStripeEvent,
  tryRecord,
};
