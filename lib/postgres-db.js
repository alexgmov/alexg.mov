...
function isHetznerRuntime() {
  return process.env.SIDESTREAM_HETZNER_RUNTIME === '1' &&
    !process.env.VERCEL &&
    !process.env.VERCEL_ENV;
}
...
function isAllowedDatabaseUrl(connectionString, environmentVariable) {
  if (!connectionString || connectionString.includes('[YOUR-PASSWORD]')) return false;
  ...
  try {
    ...
    if (environmentVariable === 'SIDESTREAM_HETZNER_POSTGRES_URL') {
      return isHetznerRuntime() && isLocalDatabaseHost(hostname);
    }
    if (hostname === 'neon.tech' || hostname.endsWith('.neon.tech')) return true;
    return isLocalDevelopment() && isLocalDatabaseHost(hostname);
  } catch {
    return false;
  }
}
...
function resolveDatabaseUrl() {
  const candidates = [
    ['SIDESTREAM_HETZNER_POSTGRES_URL', process.env.SIDESTREAM_HETZNER_POSTGRES_URL],
    ['SIDESTREAM_NEON_DATABASE_URL', process.env.SIDESTREAM_NEON_DATABASE_URL],
    ['NEON_DATABASE_URL', process.env.NEON_DATABASE_URL],
    ['DATABASE_URL', process.env.DATABASE_URL],
    ['POSTGRES_URL', process.env.POSTGRES_URL],
  ];

  return candidates.find(([name, value]) => isAllowedDatabaseUrl(value, name))?.[1] || '';
}
  ...
  return withClient(async client => {
    ...
    try {
      ...
      const existingFulfillment = await client.query(`
        ...
        limit 1
      `, [session.id]);
      if (existingFulfillment.rowCount) {
        await client.query('commit');
        return { recorded: true, alreadyFulfilled: true };
      }
...
async function isCheckoutSessionFulfilled(stripeSessionId) {
  return withClient(async client => {
    const result = await client.query(`
      select 1
      from public.purchases
      ...
      limit 1
    `, [stripeSessionId]);
    return { fulfilled: result.rowCount > 0 };
  });
}
...
async function recordPluginTelemetryBatch({ events, req, endpointVersion = '2026-06-12.1' }) {
  ...
  if (!isDatabaseConfigured()) return { skipped: true, recorded: 0 };
...
async function tryRecord(label, fn) {
  ...
  try {
    ...
    console.error(`[postgres] ${label} failed:`, err.message);
