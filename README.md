...
- `SIDESTREAM_ALLOW_LEGACY_SUPABASE_TELEMETRY`: set to `1` only for a deliberate emergency legacy Supabase REST telemetry fallback. This fallback stays lower priority than every Neon/Postgres telemetry connection and is not enabled merely by `SUPABASE_URL` plus `SUPABASE_SECRET_KEY`.
...
Do not restore the older Supabase-first telemetry writer behavior. `SUPABASE_URL`, `SUPABASE_SECRET_KEY`, and `SUPABASE_SERVICE_ROLE_KEY` are legacy for Sidestream telemetry and do not participate in the default telemetry database selection.
...
- 2026-06-12: Granted Supabase `service_role` access to Sidestream telemetry tables for the now-legacy Supabase REST writer while RLS stayed enabled and public roles stayed revoked.
- 2026-06-12: Added the now-legacy server-only Supabase REST telemetry writer using `SUPABASE_URL` plus `SUPABASE_SECRET_KEY`; the current Sidestream telemetry contract is Neon primary and may use Supabase REST only behind an explicit legacy fallback gate.
- 2026-06-12: Extended `/api/plugin-telemetry` for automatic Sidestream logging with richer redacted event envelopes, support codes, batch ids, category/severity/error/action fields, structured consent payloads, and now-legacy Supabase rollups in `sidestream_installs` plus `sidestream_sessions`.
